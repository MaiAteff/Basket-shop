        ...
        <div className="w-full text-sm text-left text-gray-500 dark:text-gray-400 space-y-4">
            ...
            return (
              <div key={item.id} className="w-full flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-b">
                ...
                <div className="flex items-center gap-2 w-full sm:w-1/4 justify-center">
                  ...
                  <button onClick={()=>updateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 border border-main rounded-full text-main">+</button>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button disabled={cartCount === 0} className="disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed w-full px-4 py-2 rounded-md bg-main text-white text-center hover:bg-[#247F75]" onClick={()=>{navigate("/checkout")}} data-discover="true">Checkout</button>
